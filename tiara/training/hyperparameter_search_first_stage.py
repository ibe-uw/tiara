...
for learning_rate in [0.001, 0.0001]:
    for dropout in [0.2]:
        architectures.append(
            dict(hid1=512, hid2=256, learning_rate=learning_rate, dropout=dropout)
        )
        architectures.append(
            dict(hid1=1024, hid2=512, learning_rate=learning_rate, dropout=dropout)
        )
        architectures.append(
            dict(hid1=2048, hid2=1024, learning_rate=learning_rate, dropout=dropout)
        )
...
with open(os.path.join(INPUT_DIR, "train/organelle.fasta"), "r") as handle:
    train_organelle = [seq for _, seq in SimpleFastaParser(handle)]
with open(os.path.join(INPUT_DIR, "train/eukarya.fasta"), "r") as handle:
    train_eukarya = [seq for _, seq in SimpleFastaParser(handle)]
with open(os.path.join(INPUT_DIR, "train/bacteria.fasta"), "r") as handle:
    train_bacteria = [seq for _, seq in SimpleFastaParser(handle)]
with open(os.path.join(INPUT_DIR, "train/archea.fasta"), "r") as handle:
    train_archea = [seq for _, seq in SimpleFastaParser(handle)]
with open(os.path.join(INPUT_DIR, "validation/organelle.fasta"), "r") as handle:
    val_organelle = [seq for _, seq in SimpleFastaParser(handle)]
with open(os.path.join(INPUT_DIR, "validation/eukarya.fasta"), "r") as handle:
    val_eukarya = [seq for _, seq in SimpleFastaParser(handle)]
with open(os.path.join(INPUT_DIR, "validation/bacteria.fasta"), "r") as handle:
    val_bacteria = [seq for _, seq in SimpleFastaParser(handle)]
with open(os.path.join(INPUT_DIR, "validation/archea.fasta"), "r") as handle:
    val_archea = [seq for _, seq in SimpleFastaParser(handle)]
...
train_X = get_tfidf_repr(
    train_organelle + train_bacteria + train_archea + train_eukarya,
    k,
...
start_time = time.time()
val_X = get_tfidf_repr(
    val_organelle + val_bacteria + val_archea + val_eukarya, k, transformer.model.idfs
)
print(f"Computing representations took: {time.time() - start_time}s.")

train_y = np.array(
    [0] * len(train_organelle)
    + [1] * len(train_bacteria)
    + [3] * len(train_archea)
    + [4] * len(train_eukarya)
)
val_y = np.array(
