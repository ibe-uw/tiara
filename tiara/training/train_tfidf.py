...
data_second_step = [
    os.path.join(DATA_PREFIX, f"{file}.fasta")
    for file in ["mitochondria", "plastids"]
]
...
for kmer in [4, 5, 6]:
    ...
    tfidf.save_params(
        params_fname=os.path.join(OUTPUT_PREF, f"k{kmer}-first-stage")
    )
...
for kmer in [4, 5, 6, 7]:
    ...
    tfidf.save_params(
        params_fname=os.path.join(OUTPUT_PREF, f"k{kmer}-second-stage")
    )
